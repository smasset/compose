...
def project_from_options(project_dir, options, additional_options={}):
    ...
    return get_project(
        ...
        ignored_config_keys=['services.build'] if '--no-build' in options else []
    )
...
def get_project(project_dir, config_path=None, project_name=None, verbose=False,
                ...
                compatibility=False, interpolate=True, ignored_config_keys=[]):
    if not environment:
    ...
    config_data = config.load(config_details, compatibility, interpolate, ignored_config_keys)
